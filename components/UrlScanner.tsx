...
import TakedownGenerator from './TakedownGenerator';
import ForensicReport from './ForensicReport';
import { createEvidencePackage, EvidencePackage } from '../services/forensicService';
...
`;
  ...
  // Helper to log scan for Dashboard
  const logScanActivity = (scannedUrl: string) => {
    try {
      const logs = JSON.parse(localStorage.getItem('sentinel_scan_log') || '[]');
      const newLog = {
      ...
      localStorage.setItem('sentinel_scan_log', JSON.stringify(updatedLogs));
    } catch (e) {
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6 animate-fade-in">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div className="space-y-4">
                        ...
                        onClick={async () => {
                          if (analysis) {
                            const pkg = await createEvidencePackage(
                              ...
                              {
                                nudityScore: analysis.matchResult.nudityScore || 0,
                                riskScore: analysis.matchResult.nudityScore || 0, // Using nudityScore as riskScore proxy
                                confidence: analysis.matchResult.confidence,
                                method: analysis.matchResult.detectionMethod || 'unknown',
                                reasoning: analysis.matchResult.reasoning
                              },
                              analysis.thumbnailUrl?.startsWith('data:') ? analysis.thumbnailUrl.split(',')[1] : undefined
                            );
                            setEvidencePackage(pkg);
                            setShowForensicModal(true);
                          }
                        }}
                        className="w-full bg-green-700 hover:bg-green-600 text-white py-3 rounded-lg text-sm font-medium transition-colors flex items-center justify-center space-x-2"
                    >
                        <Icons.Shield />
                    ...
                    <button 
                        ...
                        className="w-full bg-red-700 hover:bg-red-600 text-white py-3 rounded-lg text-sm font-medium transition-colors flex items-center justify-center space-x-2"
                    >
                        <Icons.Legal />
                        <span>Generate DMCA Takedown</span>
                    </button>
                </div>
            </div>
        </div>
      ...
      {showForensicModal && evidencePackage && (
        <ForensicReport
