  ...
  return (
    <Link 
      ...
      className={`flex items-center space-x-3 px-4 py-3 rounded-lg transition-all ${
        active ? 'bg-brand-900/30 text-brand-400 border border-brand-800/50' : 'text-slate-400 hover:text-white hover:bg-slate-800'
      }`}
    >
      <div className={`${active ? 'text-brand-400' : 'text-slate-500'}`}>{icon}</div>
    ...
    <div className="min-h-screen bg-slate-950 flex">
        ...
        <div className="p-4 border-t border-slate-800">
          <div className="flex items-center space-x-3 px-4 py-2 bg-slate-850 rounded-lg border border-slate-700">
            ...
            <div className="flex flex-col">
              <span className="text-[10px] text-slate-400 uppercase font-semibold">Connection</span>
      ...
      <main className="flex-1 md:ml-64 p-4 md:p-8 overflow-x-hidden">
        ...
        <div className="md:hidden flex items-center justify-between mb-6 pb-4 border-b border-slate-800">
          ...
          <div className="text-xs text-brand-500 bg-brand-900/20 px-2 py-1 rounded">Secure Mode</div>
        </div>
          ...
          <Route path="/" element={
            <div className="animate-fade-in">
              <header className="mb-8">
                ...
                <p className="text-slate-400 text-sm mt-1">Real-time threat monitoring and asset protection status.</p>
              </header>
              <Dashboard />
            </div>
          } />
          <Route path="/vault" element={
             <div className="animate-fade-in max-w-3xl mx-auto mt-8">
                ...
                <div className="mt-8 p-6 bg-slate-900/50 rounded-lg border border-slate-800">
                  <h4 className="text-sm font-semibold text-slate-300 mb-2">Architectural Note</h4>
                  <p className="text-xs text-slate-500 leading-relaxed">
                    This module demonstrates the <strong>Privacy by Design</strong> principle. 
                    Video processing (keyframe extraction and hashing) occurs within the browser (Client-Side) via WebAssembly. 
                    Only the non-reversible unique fingerprint (hash) is sent to the backend Vector Database (Milvus). 
                    The original content never leaves the victim's device.
                  </p>
                </div>
             </div>
          } />
          ...
          <Route path="/matches" element={
            <div className="animate-fade-in h-full">
               <header className="mb-6">
                ...
                <p className="text-slate-400 text-sm mt-1">Review detections and authorize legal takedowns.</p>
              </header>
              <MatchReview />
            </div>
          } />
